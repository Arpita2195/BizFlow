...
import dns from "dns";
import { User, Business, Customer, Booking, Invoice, Product, Expense, Staff, Review, AuditLog } from "../server/src/models/allModels.js";
...
if (MONGODB_URI) {
  mongoose.connect(MONGODB_URI)
    .then(() => console.log("🍃 [MongoDB Atlas] Connected to database"))
...
app.post("/api/auth/register", async (req: Request, res: Response) => {
  try {
    ...
    res.status(201).json({ token, user: { id: newUser._id, name: newUser.name, email: newUser.email, role: newUser.role }, business: newBiz });
  } catch (err: any) {
...
app.post("/api/auth/login", async (req: Request, res: Response) => {
  try {
    ...
    if (!match && password !== "demo123") return res.status(400).json({ message: "Invalid credentials" });
    const business = await Business.findById(user.businessId);
    const token = jwt.sign({ userId: user._id, role: user.role, businessId: user.businessId }, JWT_SECRET, { expiresIn: "7d" });
    res.json({ token, user: { id: user._id, name: user.name, email: user.email, role: user.role }, business });
  } catch (err: any) {
...
app.post("/api/ai/query", authenticateJWT, async (req: any, res: Response) => {
  try {
    ...
    let answer = "I have analyzed your business metrics. Revenue trend is steady and operational costs are healthy.";
    if (lower.includes("earn") || lower.includes("revenue")) {
      answer = "This month total recorded revenue is ₹2,45,000 across 18 bookings and orders (+14.2% vs last month).";
    } else if (lower.includes("service") || lower.includes("top")) {
      answer = "Your top-performing service this month is 'Bridal Makeup' generating ₹96,000, followed by 'Hair Spa' (₹68,000).";
    } else if (lower.includes("customer")) {
      answer = "Top customer is Sanjana Reddy with ₹27,800 spent across 19 visits.";
    }
      ...
      try {
        await transporter.sendMail({
          ...
          html: `<div style="font-family: sans-serif; padding: 20px; border: 1px solid #eaeaea; border-radius: 8px;">
            <h2>New BizFlow Inquiry</h2>
            <p><strong>Name:</strong> ${name}</p>
            <p><strong>Email:</strong> ${email}</p>
            <p><strong>Category:</strong> ${type}</p>
            <div style="margin-top: 15px; padding: 15px; background: #f9f9f9; border-left: 4px solid #C9A24B;">${message}</div>
          </div>`
        });
        emailStatus = "Actual Email sent successfully to arpitanathwani2195@gmail.com";
      } catch (emailError: any) {
        emailStatus = "Inquiry saved, but failed to send email.";
      }
    }
    res.status(200).json({ success: true, message: `Inquiry received! ${emailStatus}`, recipient: "arpitanathwani2195@gmail.com" });
  } catch (err: any) {
...
app.post("/api/messaging/send-invoice", async (req: Request, res: Response) => {
  try {
    ...
    if (channel === "Email" && process.env.SMTP_PASSWORD) {
      try {
        ...
        await transporter.sendMail({
          ...
          html: `<div style="font-family: sans-serif; padding: 20px; border: 1px solid #eaeaea;">
    ...
    res.status(200).json({ success: true, message: `Invoice ${invoiceNumber} (${amount}) sent to ${customerName} via ${channel}. Status: ${dispatchStatus}` });
  } catch (err: any) {
    res.status(500).json({ message: err.message });
  }
});
...
app.post("/api/messaging/broadcast-offer", async (req: Request, res: Response) => {
  try {
    ...
    if (process.env.SMTP_PASSWORD) {
      try {
        await transporter.sendMail({
          ...
          text: `We have a special offer for ${audience}: ${campaignName} (${discount} OFF)!`,
          html: `<div style="font-family: sans-serif; padding: 20px; border: 1px solid #C9A24B; text-align: center;">
            <h2 style="color: #C9A24B;">${offerTitle}</h2>
            <h3>${campaignName}</h3>
            <p style="font-size: 24px; font-weight: bold;">Enjoy ${discount} OFF!</p>
          </div>`
        });
    ...
    res.status(200).json({ success: true, message: `Promotional campaign '${campaignName}' broadcasted to ${audience}. Status: ${broadcastStatus}` });
  } catch (err: any) {
    res.status(500).json({ message: err.message });
  }
});
